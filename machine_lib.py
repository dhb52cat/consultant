...
def login():
    ...
    username = "dhb52cat@163.com"
    password = "WNiAurR7qAbo0"
...
def process_datafields(df, data_type):
    ...
    return tb_fields
def process_datafieldsfromlocal(datafields):

    tb_fields = []
    for field in datafields:
        tb_fields.append("winsorize(ts_backfill(%s, 120), std=4)"%field)
    return tb_fields
def process_datafieldsfrommodel(A,B,C,model):
    tb_fields = []
    for field in A:
        str=model.replace('A',field)
        for fieldB in B:
            strb=str.replace('B',fieldB)
            # for fc in C:
            #     strb=str.replace('B',fieldB).replace('C',fc)
            tb_fields.append(strb)
        # tb_fields.append("winsorize(ts_backfill(%s, 120), std=4)"%field)
...
def get_alphas(start_date, end_date, sharpe_th, fitness_th, region, alpha_num, usage):
    ...
    output_dict = {"next" : next_alphas, "decay" : decay_alphas,"alpha_list":a_list}
    print("count: %d"%count)
    return output_dict
